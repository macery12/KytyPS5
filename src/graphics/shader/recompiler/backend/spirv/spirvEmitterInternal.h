...
uint32_t AtomicUpdate(EmitterState& state, uint32_t pointer, IR::ResourceKind kind, Fn&& desired) {
	...
	state.builder.AddFunction(spv::OpPhi, TypeU32(state), observed, initial, preheader, exchanged,
	                          cont);
	state.builder.AddFunction(spv::OpPhi, TypeU32(state), retries, ConstantU32(state, 0),
	                          preheader, next_retry, cont);
	...
	state.builder.AddFunction(spv::OpIAdd, TypeU32(state), next_retry, retries,
	                          ConstantU32(state, 1));
	state.builder.AddFunction(spv::OpUGreaterThanEqual, TypeBool(state), exhausted, next_retry,
	                          ConstantU32(state, RetryLimit));
	state.builder.AddFunction(spv::OpLogicalOr, TypeBool(state), done, success, exhausted);
